...
Component({
  properties: {
    pageType: {
      type: String,
      value: 'identification', // identification: 查找车型页没有查询全部操作，part: 选择车型页有查询全部操作
    },
    showModal: {// 选择车型弹窗的隐藏与显示
      type: Number,
      value: 0,
    },
    selectModelState: {// 是否存在当前选择的车型
      type: Number,
    },
    showSelectModel: {
      type: Boolean,
  ...
  methods: {
    ...
    // 根据关键字进行车系搜索
    searchSeriesFn(query) {
      this.setData({
        showSeriesResult: true,
        noMoreResult: false,
      })
      services._request(`/home/search?kw=${query}`).then((res) => {
        console.log(res);
        let data = res.result;
        if(res.success){
          let seriesResult = [];
    ...
    selectSearchSeriesFn(e) {
      let {series_id, name} = e.currentTarget.dataset;
      // let reg = /\b(.*)\b\s\b(.*)/;
    ...
    // 关闭车型选择
    closeSlectModelFn() {
      this.closeYearAndDispModelFn();
      if (this.data.pageState === 'brand') {
        this.triggerEvent('deleteSelectModelFn', {});
      }
      this.setData({
        pageState: 'series',
        showModal: 0,
      ...
      this.triggerEvent('selectModelInit',{
        showSelectModel: false,
      });
      if (JSON.stringify(this.data.vehicle) === '{}'){
        this.setData({
          showModal: 0,
        })
        return;
      ...
      let year = this.data.vehicle.year ? this.data.vehicle.year : '';
      let displacement = this.data.vehicle.displacement ? this.data.vehicle.displacement : '';
      let seriesId = this.data.vehicle.series_id ? this.data.vehicle.series_id : '';
      // let modelId = this.data.model_id ? this.data.model_id : '';&& !modelId
      
      if (!year && !displacement && seriesId ){
        this.getPartBySeriesFn();
      }else{
        this.setData({
          showModal: 0,
        })
    ...
    handlerAlphaTap(e) {
      ...
      this.setData({ alpha: ap.substr(0, 1) });
    },
  },
